...
export default function DictionaryIndexPage() {
  ...
  const celestialMap = useMemo(() => {
    ...
    return constellations.map(c => {
      const s = c.season || '';
      const realCoordKey = Object.keys(REAL_COORDS).find(k => c.name.includes(k) || k.includes(c.name));
      
      let ra = 0, dec = 0;

      if (realCoordKey) {
        ra = REAL_COORDS[realCoordKey].ra;
        dec = REAL_COORDS[realCoordKey].dec;
      } else {
      ...
      while(true) {
        ...
        const collision = targetList.find(p => {
          ...
          return dra < 0.8 && dy < 8; // SVGを表示するためスペースを少し広めに確保
        });
  ...
  const displayStars = useMemo(() => {
    ...
    return celestialMap.map(c => {
      ...
      // ★ 空を見上げるプラネタリウムの視点（左が東、右が西）に反転修正！
            ...
            return (
              ...
              >
                <div className={`p-4 rounded-full flex flex-col items-center justify-center transition-all duration-700 ${isCurrent ? 'opacity-100 scale-110' : isAdjacent ? 'opacity-60 scale-90 hover:opacity-100 hover:scale-100' : 'opacity-20 scale-75 hover:opacity-100 hover:scale-90'}`}>
                  ...
                  {/* ★ SVG（本物の星座の形）をドットの背面に透過表示 */}
                  {svgContent && (
                    <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 pointer-events-none transition-all duration-700 ${isCurrent ? 'opacity-40' : 'opacity-10'}`}>
                      <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible stroke-white fill-transparent stroke-[1px] drop-shadow-[0_0_5px_rgba(255,255,255,0.8)]">
                        {svgContent}
                      </svg>
                    </div>
                  )}
                  ...
                  <span className={`mt-2 whitespace-nowrap text-[11px] font-bold px-3 py-1 rounded-md backdrop-blur-md border transition-all duration-300 relative z-20 ${
            ...
            return (
              ...
              >
                <div className="p-3 flex flex-col items-center opacity-40 hover:opacity-100 transition-opacity duration-300">
                  ...
                  {/* 南天もSVGを表示 */}
                  {svgContent && (
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 pointer-events-none opacity-20 group-hover:opacity-50 transition-opacity">
                  ...
                  <div className={`rounded-full transition-all duration-300 relative z-10 ${isSelected ? 'w-4 h-4 bg-emerald-400 shadow-[0_0_15px_#34d399]' : 'w-2 h-2 bg-white/70'}`} />
                  <span className={`mt-1.5 whitespace-nowrap text-[10px] font-bold px-2 py-0.5 rounded bg-black/50 border transition-all relative z-20 ${isSelected ? 'text-emerald-400 border-emerald-400/50' : 'text-white/50 border-transparent group-hover:border-white/20'}`}>
                    {c.name}
                  </span>
                </div>
              </button>
