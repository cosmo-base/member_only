// app/shittoku/feedback/feedback-client.tsx
"use client"
...
export function ShittokuFeedbackClient({ recentEventNames }: FeedbackClientProps) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isSuccess, setIsSuccess] = useState(false)

  // フォームの入力状態を管理
  const [formData, setFormData] = useState({
    // ★ 抽出したイベントの最新のもの（1番目）をデフォルト選択にする
    eventName: recentEventNames[0], 
    satisfaction: "",
    satisfactionReason: "",
    purposes: [] as string[],
    purposeReason: "",
              ...
              {[
                ...
                <label key={option} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${formData.purposes.includes(option) ? 'border-primary bg-primary/10' : 'border-border/50 bg-secondary/20 hover:bg-secondary/50'}`}>
                  <input
                    ...
                    className="mt-0.5 w-4 h-4 text-primary rounded bg-background border-border focus:ring-primary/50 shrink-0"
                  />
